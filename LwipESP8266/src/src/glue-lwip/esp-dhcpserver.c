...
static void ICACHE_FLASH_ATTR send_offer(struct dhcps_msg *m)
{
        ...
        if (!lwiperr_check("send_offer", udp_sendto( pcb_dhcps, p, &broadcast_dhcps, DHCPS_CLIENT_PORT ))){
#if DHCPS_DEBUG
...
static void ICACHE_FLASH_ATTR send_nak(struct dhcps_msg *m)
{
        ...
        lwiperr_check("dhcps send nak", udp_sendto( pcb_dhcps, p, &broadcast_dhcps, DHCPS_CLIENT_PORT ));
        if(p->ref != 0){
#if DHCPS_DEBUG
...
{
    // was: struct netif * apnetif = (struct netif *)eagle_lwip_getif(0x01);
    // but we are now in the lwip2 side so we deal with lwip2 structs
    // where netif->dhcps_pcb does not exist
    // we can use netif->state which is unused
