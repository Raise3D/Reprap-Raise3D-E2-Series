...
void Platform::Init()
{
	if (DiagPin != NoPin)
	{
		;//pinMode(DiagPin, OUTPUT_HIGH);				// set up diag LED for debugging and turn it off
	}

	// Deal with power first (we assume this doesn't depend on identifying the board type)
	pinMode(ATX_POWER_PIN, OUTPUT_HIGH);
	pinMode(E6_EN_PIN, OUTPUT_LOW);//E6_EN_PIN   LED 
	pinMode(E6_STEP_PIN, OUTPUT_LOW);//E6_EN_PIN  12V control  low --12v ourtput
	pinMode(E6_DIR_PIN, OUTPUT_HIGH);//E6_EN_PIN
...
# endif
	...
	default:
		numSmartDrivers = 6;									// assume that any additional drivers are dumb enable/step/dir ones
		break;
	}
#endif
...
void Platform::InitZProbe()
{
	...
	default:
		...
		pinMode(zProbeModulationPin, OUTPUT_HIGH);		// we now set the modulation output high during probing only when using probe types 4 and higher
		break;
	}
}
...
{
	if (c != Compatibility::me && c != Compatibility::reprapFirmware && c != Compatibility::marlin && c != Compatibility::nanoDLP && c!=Compatibility::raise)
	{
...
# endif
			...
			if (stalledDriversToLog != 0 && reprap.GetGCodes().IsReallyPrinting())
			{
				...
				MessageF(UsbMessage, "warning:Driver(s)%s stalled at Z height %.2f\n", scratchString.c_str(), (double)liveCoordinates[Z_AXIS]);
				reported = true;
			}
#endif
...
{
	const size_t numAxes = reprap.GetGCodes().GetTotalAxes();
//	EndStopInputType tempendstop;
//	tempendstop=endStopInputType[drive];
//	if(homezflag==1 && drive==2)
//		tempendstop=EndStopInputType::activeHigh;
	if (drive < numAxes)
	{
		switch (endStopInputType[drive])////tempendstop
		{
...
void Platform::RebootPower()
{
	digitalWrite(E6_STEP_PIN, HIGH);
	digitalWrite(E6_STEP_PIN, LOW);
}
bool Platform::AtxPower() const
{
	return IoPort::ReadPin(E6_EN_PIN); //IoPort::ReadPin(ATX_POWER_PIN);
}

void Platform::AtxPowerOn()
{
	deferredPowerDown = false;
	//IoPort::WriteDigital(ATX_POWER_PIN, true); // Raise
	IoPort::WriteDigital(E6_EN_PIN, true);
	IoPort::WriteDigital(E6_STEP_PIN, true);
}
...
void Platform::AtxPowerOff(bool defer)
{
	...
	if (!defer)
	{
		...
		//IoPort::WriteDigital(ATX_POWER_PIN, false);// Raise
		IoPort::WriteDigital(E6_EN_PIN, false);
		IoPort::WriteDigital(E6_STEP_PIN, false);
	}
}
