...
//#define RAISE_NEW_PROBE_TEST
//#define RAISE_LARGE_MACHINE
//#define STOP_PID_PROTECTION
//#define Raise_CMD_DEBUG
#define DUPLICATE_MIRROR_TOOL 2
#define RIGHT_X_CARRIAGE_TOOL 1
#define LEFT_X_CARRIAGE_TOOL 0
#define USB_BREAK
#define MOVE_COUNTER (reprap.GetMove().GetScheduledMoves() - reprap.GetMove().GetCompletedMoves())

const char feedrateLetter = 'F';						// GCode feedrate
const char extrudeLetter = 'E'; 						// GCode extrude
extern char homezflag;
...
public:
	struct RawMove
	{
		...
		uint8_t isZMoving : 1;											// True if z moving (exclude Bed Mesh)
	...
	void InitRaiseMessages();											// send init messages for Raise3D
	void Init();														// Set it up
	...
	void GetRaiseCurrentCoordinates(const StringRef& s) const;
	...
	void CurrentMoveCompleted(bool isZMoving);									//Raise current move completetd
	void initProcessID(); 														//init process id
...
private:
	...
	void GenerateRaiseTemperatureReport(const StringRef& reply) const;      //Raise
	...
	bool IsXUAxisCrash(float x, float u);								// Raise
	bool RaiseToolFree(int toolNum);								// Raise tool free command
	void updateFilamentStatus(int toolNum);							// Raise Update filament status for mirror and duplicate mode
	bool isUAXISVisible();
	bool isCurrentUAXIS();
	float GetRaiseCurrentZPos();									//Get Current z pos
	bool writeProcessIDToFile();
	bool readProcessIDFromFile();
	...
	int filterFilamentError[MaxExtruders]; //filter filament error to prevent from hardware detection error
	int frsCheckTimes[MaxExtruders];		//record frs sensor trigger check times
	FilamentSensorStatus lastFilamentStatus[MaxExtruders];//Raise last Filament sensor status
	bool frsTriggerCheck[MaxExtruders];	//record frs extruder check
	...
	bool isRaiseHomeZProbe;						//raise home z by z probe
	bool isRaiseZHoming;						//raise check homing z
	bool useMeshBeforeHomingZ;					//raise check use mesh before homing z
	PrintMode raisePrintMode;							// S0-normal S1-duplicate S2-mirror
	bool isRaiseToolFreeMove;						// raise tool free movement
	bool isRaiseUsbHeart;						// usb break heart
	bool isZMovingCompleted;					// Raise true if current z move completed
	uint32_t raiseIdleTime;						//idle time since last move
	...
	static constexpr const char* PROCESS_ID_G = "processid.g";
#if HAS_SMART_DRIVERS
