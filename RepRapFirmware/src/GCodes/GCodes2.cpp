...
bool GCodes::ActOnCode(GCodeBuffer& gb, const StringRef& reply)
{
...
#ifdef RAISE_DEBUG1
			String<256> raisestr;
			raisestr.printf("execute #1: %s\n", gb.Buffer());
			platform.MessageF(UsbMessage, raisestr.c_str());
#endif
	...
	case 'M':
		...
		//Raise
		else if (platform.Emulating() == Compatibility::raise)
		{
#ifdef RAISE_DEBUG1
			String<256> raisestr;
			raisestr.printf("execute #3: %s\n", gb.Buffer());
			platform.MessageF(UsbMessage, raisestr.c_str());
#endif
...
bool GCodes::HandleGcode(GCodeBuffer& gb, const StringRef& reply)
{
	...
	case 1: // Ordinary move
		{
			...
			{
#ifdef RAISE_DEBUG2
				platform.MessageF(UsbMessage, "segmentleft: %u\n", segmentsLeft);
#endif
				return false;
			}
			if (!LockMovement(gb))
			{
#ifdef RAISE_DEBUG2
				platform.MessageF(UsbMessage, "can not lock movement\n");
#endif
				return false;
			}
			{
	...
	case 30: // Z probe/manually set at a position and set that as point P
		...
		if (Compatibility::raise != reprap.GetPlatform().Emulating() && (reprap.GetMove().GetKinematics().AxesToHomeBeforeProbing() & ~axesHomed) != 0)
		{
...
bool GCodes::HandleMcode(GCodeBuffer& gb, const StringRef& reply)
{
	...
	case 28: // Write to file
		{
			...
			if (gb.GetUnprecedentedString(filename.GetRef()))
			{
				//raise use M28 to refresh config files
				const bool ok = gb.OpenFileToWrite(Compatibility::raise == reprap.GetPlatform().Emulating()?platform.GetSysDir(): platform.GetGCodeDir(),
						filename.c_str(), 0, false, 0);
				if (ok)
				{
	...
	case 106: // Set/report fan values
		{
			...
			if(!seenFanNum && platform.Emulating() == Compatibility::raise)
			{
				if (gb.Seen('S'))
				{
					const float f = ConvertOldStylePwm(gb.GetFValue());
					platform.SetFanValue(0, f);
					platform.SetFanValue(1, f);
				}
			}
			else
			{
	...
	case 104:
		...
		// M104 does the same but doesn't ever select a tool
		{
			...
			{
				if (platform.Emulating() != Compatibility::raise) reply.copy("Invalid tool number");
				if (platform.Emulating() != Compatibility::raise) result = GCodeResult::error;
				break;
			}
			...
			{
#ifdef RAISE
				//to prevent nozzle cooling from tool switching during mirror and duplicate mode
				//as standby temperature just set to tool2
				if(2 == applicableTool->Number())
				{
					if(nullptr != reprap.GetTool(0))
						SetToolHeaters(reprap.GetTool(0), temperature, true);
					if(nullptr != reprap.GetTool(1))
						SetToolHeaters(reprap.GetTool(1), temperature, true);
				}
#endif
	...
	case 114://Raise
		if (platform.Emulating() == Compatibility::raise)
		{
			GetRaiseCurrentCoordinates(reply);
		}else
		{
	...
	case 191: // Set chamber temperature and wait
		...
		}
		{
			...
			if (heater >= 0)
			{
				...
				if (cancelWait || reprap.GetHeat().HeaterAtSetTemperature(heater, waitWhenCooling, tolerance))
				{
					cancelWait = isWaiting = false;
					if (platform.Emulating() == Compatibility::raise)
					{
						//raise
						platform.MessageF(MessageType::UsbMessage, "ok:%" PRIu32 "\n", reprap.GetMove().GetScheduledMoves() - reprap.GetMove().GetCompletedMoves());
					}
					break;
				}
				...
				if (code == 190)
				{
					gb.SetState(GCodeState::m190WaitForBed);
				}
	...
	case 208: // Set/print maximum axis lengths. If there is an S parameter with value 1 then we set the min value, else we set the max value.
		{
			...
			raiseDuplicateOffset = (platform.AxisMaximum(U_AXIS)+platform.AxisMinimum(X_AXIS)) * 0.5;
		}
		break;
	...
	case 221:	// Set/report extrusion factor override percentage
		{
			int32_t extruder = 0;
			bool dummy;
			gb.TryGetIValue('T', extruder, dummy);
			...
			if (extruder >= 0 && extruder < (int32_t)numExtruders)
			{
				if (gb.Seen('S'))	// S parameter sets the override percentage
				{
					...
					{
#ifdef RAISE
						if(DUPLICATE_MIRROR_TOOL == reprap.GetCurrentToolNumber())
						{
							for(extruder = 0; extruder < 2; extruder++)
							{
								if (segmentsLeft != 0 && !moveBuffer.isFirmwareRetraction)
								{
	...
	case 280:	// Servos
		...
		if (gb.Seen('S'))
		{
			int32_t angleOrWidth = gb.GetIValue();
			if(angleOrWidth==200)
				platform.SetProbing(true); //newprobe
			if(angleOrWidth==10)
				platform.SetProbing(false); //newprobe
		}
		break;
#else
		if (gb.Seen('P'))
	...
	case 303: // Run PID tuning
		if (gb.Seen('H'))
		{
			...
			{
				if(Compatibility::raise == reprap.GetPlatform().Emulating())
					reply.printf("PID Autotune failed! Bad heater number in M303 command");
				else
			...
			{
				if(Compatibility::raise == reprap.GetPlatform().Emulating())
					reply.printf("PID Autotune failed! Heater is not ready to perform PID auto-tuning");
				else
			...
			{
				if(Compatibility::raise == reprap.GetPlatform().Emulating())
				reply.printf("PID Autotune failed! Invalid PWM in M303 command");
				else
	...
	case 355://Raise3D turn on and turn off lights
		{
			if(gb.Seen('S'))
			{
				const int flag = gb.GetIValue();
				if(0 == flag)
	...
	case 400: // Wait for current moves to finish
		...
		if(isRaiseToolPosRestore)
		{
			//restore z axis
			SetMoveBufferDefaults();
			moveBuffer.coords[Z_AXIS] = toolChangeRestorePoint.moveCoords[Z_AXIS];
			moveBuffer.feedRate = toolChangeRestorePoint.feedRate;
	...
	case 405://raise
		{
			reprap.SetFilamentSensorSwitch(true);
		}
		break;
	case 406://raise
		{
			reprap.SetFilamentSensorSwitch(false);
		}
		break;
	...
	case 555: // Set/report firmware type to emulate
		...
		else
		{
			...
			case Compatibility::raise:
				noMovesBeforeHoming = false;//Enable Z Movement to ensure power loss resuming is working.
				reply.cat("Raise");
				break;
	...
	case 605:
		...
		break;
	case 990:
		// check the filament monitor status
		result = GCodeResult::ok;
		reply.printf("left:%d,right:%d",(int)lastFilamentStatus[0], (int)lastFilamentStatus[1]);
		break;
	case 1001:
	case 1002:
	case 2000:
	...
	case 3001:
		...
		break;
	case 3002:
		isRaiseUsbHeart = false;
		result = GCodeResult::ok;
		break;
	case 3003:
		if (gb.Seen('S'))
		{
			float offset = gb.GetFValue();
			if(offset >= (platform.AxisMinimum(U_AXIS) - platform.AxisMinimum(X_AXIS)))
			{
				raiseDuplicateOffset = offset;
			}
		}
		result = GCodeResult::ok;
		break;
...
bool GCodes::HandleTcode(GCodeBuffer& gb, const StringRef& reply)
{
	...
	if (seen)
	{
		...
		if (oldTool == nullptr || oldTool->Number() != toolNum)
		{
			...
			if(gb.Seen('S'))//if S0 ignore file macro for tool change
			{
				if(0 == gb.GetUIValue())
				{
					gb.MachineState().ignore_FileMacro = true;
				}
			}

			if(DUPLICATE_MIRROR_TOOL != toolNum)
				gb.SetState(GCodeState::toolChange0);
			else
			{
				//duplicate and mirror tool dont need to additional operations
				reprap.SelectTool(gb.MachineState().newToolNumber, simulationMode != 0);
				ToolOffsetInverseTransform(moveBuffer.coords, currentUserPosition);
				reply.printf("echo:Active Extruder: %u", toolNum);
				HandleReply(gb, GCodeResult::ok, reply.c_str());
				//duplicate and mirror tool need to check two filament monitor status
				updateFilamentStatus(toolNum);
			}
