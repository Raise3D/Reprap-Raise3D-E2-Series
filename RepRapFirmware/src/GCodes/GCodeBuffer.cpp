...
bool GCodeBuffer::Put(char c)
{
	...
	do
	{
		...
		case GCodeBufferState::parsingLineNumber:			// we saw N at the start and we are parsing the line number
			...
			{
				AddToChecksum(c);
				if (isDigit(c)) lineNumber = (10 * lineNumber) + (c - '0');
				if (lineNumberNegative)  //Raise: Add negative line number support
				{
					lineNumber = lineNumber * -1;
					lineNumberNegative = false;
				}
				if (c == '-')
				{
					lineNumberNegative = true;
				}
				break;
			}
			else
...
float GCodeBuffer::GetFValue()
{
	if (readPointer >= 0)
	{
		...
		{
			is_negative = -1;
			readPointer += 1; //Raise: Support negative values;
		}
		const float result = SafeStrtof(&gcodeBuffer[readPointer + 1], 0) * is_negative;
		readPointer = -1;
		return result;
	}
...
void GCodeBuffer::WriteToFile()
{
	if (GetCommandLetter() == 'M')
	{
		if (GetCommandNumber() == 29)						// end of file?
		{
			...
			SetFinished(true);
			const char* const r = ((reprap.GetPlatform().Emulating() == Compatibility::marlin) || (reprap.GetPlatform().Emulating() == Compatibility::raise)) ? "Done saving file." : "";
