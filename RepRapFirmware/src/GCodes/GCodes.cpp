	...
	isFlashing(false), fileBeingHashed(nullptr), lastWarningMillis(0), sdTimingFile(nullptr)
{
	...
	serialGCode = new GCodeBuffer("serial", UsbMessage, false);//Raise
#if HAS_NETWORKING
...
void GCodes::Init()
{
	...
	active = true;
	limitAxes = true;
	noMovesBeforeHoming = false; //Raise: allow move before homing to ensure loss recovery function.
	SetAllAxesNotHomed();
...
#endif
	isRaiseHomeZProbe = true;
	isRaiseZHoming = false;
	useMeshBeforeHomingZ = true;
	raisePrintMode = PrintMode::normal;;
	isRaiseToolFreeMove = false;
	isRaiseToolPosRestore = false;
	isZMovingCompleted = false;
	raiseIdleTime = millis();
	isRaiseUsbHeart = true;
	for(size_t i= 0; i < MaxExtruders; ++i)
	{
		filterFilamentError[i] = 0;
		frsCheckTimes[i] = 0;
...
void GCodes::initProcessID()
{
	/////////////init processID
	processID = 0;
	readProcessIDFromFile();
	processID = processID > 9999? 0 : processID;
	writeProcessIDToFile();
	////////////////////////////////
}

#define MAX_INT_LENGHT 12
bool GCodes::writeProcessIDToFile()
{
	FileStore * const f = platform.OpenFile(platform.GetSysDir(), PROCESS_ID_G, OpenMode::write);
	if (f == nullptr)
	{
		return false;
	}
	String<MAX_INT_LENGHT> indexStr;
	indexStr.printf("%d", (int)(processID+1));
	FileData writeData;
	writeData.Set(f);
	writeData.Write(indexStr.c_str());
	writeData.Close();
	return true;
}

bool GCodes::readProcessIDFromFile()
{
	FileStore * const f = platform.OpenFile(platform.GetSysDir(), PROCESS_ID_G, OpenMode::read);
	if (f == nullptr)
	{
		return false;
	}
	FileData readData;
	readData.Set(f);
	char indexStr[MAX_INT_LENGHT]= {0};
	readData.Read(indexStr, MAX_INT_LENGHT);
	readData.Close();
	processID = SafeStrtol(indexStr);
	return true;
}


void GCodes::InitRaiseMessages()
{
	platform.MessageF(UsbMessage, "start!\n");
	platform.MessageF(UsbMessage, "SYSTEM_VERSION:%s\n", VERSION);
	platform.MessageF(UsbMessage, "FIRMWARE_VERSION:%s\n", RAISEVERSION);
	platform.MessageF(UsbMessage, "BOARD_VERSION:Duet_V2\n");
	platform.MessageF(UsbMessage, "PROCESSID:%ld\n", processID);
	String<256> reply;
	reply.copy("echo: M92 ");
	...
	{
		reply.catf("%c%u:%.3f ", sep, extruder, (double)platform.DriveStepsPerUnit(extruder + numTotalAxes));
	}
	reply.catf("\n");
	platform.MessageF(UsbMessage, reply.c_str());

	reply.CopyAndPad("");
	reply.copy("echo: M201 ");
	for (size_t axis = 0; axis < numTotalAxes; ++axis)
	{
		reply.catf("%c%.2f ", axisLetters[axis], (double)(platform.Acceleration(axis) / distanceScale));
	}
	reply.catf("%c%.2f", sep,(double)(platform.Acceleration(numTotalAxes) / distanceScale));
	reply.catf("\n");
	platform.MessageF(UsbMessage, reply.c_str());

	reply.CopyAndPad("");
	const Move &move = reprap.GetMove();
	reply.catf("echo: M204 P%.2f T%.2f\n", (double)move.GetMaxPrintingAcceleration(), (double)move.GetMaxTravelAcceleration());
	platform.MessageF(UsbMessage, reply.c_str());

	reply.CopyAndPad("");
	reply.copy("echo: M205 ");
	for (size_t axis = 0; axis < numTotalAxes; ++axis)
	{
		reply.catf("%c%.2f ", axisLetters[axis], (double)(platform.GetInstantDv(axis) / distanceScale));
	}
	reply.catf("%c%.2f", sep, (double)(platform.GetInstantDv(numTotalAxes) / distanceScale));
	reply.catf("\n");
	platform.MessageF(UsbMessage, reply.c_str());


	for (size_t extruder = 1; extruder < numExtruders + 1; extruder++)
	{
		reply.CopyAndPad("");
		reply.copy("echo: M301 ");
		const FopDt& model = reprap.GetHeat().GetHeaterModel(extruder);
		M301PidParameters pp = model.GetM301PidParameters(false);
		reprap.GetHeat().SetM301PidParameters(extruder, pp);
		reply.catf("e:%u p:%.2f i:%.2f d:%.2f", extruder, (double)pp.kP, (double)pp.kI, (double)pp.kD);
		reply.catf("\n");
		platform.MessageF(UsbMessage, reply.c_str());
	}

	reply.CopyAndPad("");
	reply.copy("echo: M208 ");
	for (size_t axis = 0; axis < numVisibleAxes; ++axis)
	{
		reply.catf("%c%.2f ", axisLetters[axis], (double)platform.AxisMaximum(axis));
	}
	reply.catf("\n");
	platform.MessageF(UsbMessage, reply.c_str());
	...
	reply.catf("X%.2f ", (double)params.xOffset);
	reply.catf("Y%.2f ", (double)params.yOffset);
	reply.catf("Z%.2f ", (double)params.triggerHeight);
	reply.catf("\n");
	platform.MessageF(UsbMessage, reply.c_str());


	platform.MessageF(UsbMessage, "init finished!\n");
}
...
void GCodes::Reset()
{
	...
	for(size_t i= 0; i < MaxExtruders; ++i)
	{
		lastFilamentStatus[i] = FilamentSensorStatus::ok;
	}
...
void GCodes::RunStateMachine(GCodeBuffer& gb, const StringRef& reply)
{
...
#ifdef RAISE
				if(0 == strcmp(nextHomingFileName.c_str(),"homez.g") || 0 == strcmp(nextHomingFileName.c_str(),"homeall.g"))
				{
					if(isRaiseHomeZProbe)
					{
						String<RepRapPasswordLength> probeZName = nextHomingFileName;
						nextHomingFileName.copy("probe_");
						nextHomingFileName.cat(probeZName.c_str());
					}
					isRaiseZHoming = true;
					useMeshBeforeHomingZ = reprap.GetMove().IsUsingMesh();
				}
#endif
	...
	case GCodeState::homing2:
		if (LockMovementAndWaitForStandstill(gb))		// movement should already be locked, but we need to wait for the previous homing move to complete
		{
			...
			else
			{
				toBeHomed &= ~axesHomed;
#ifdef RAISE
				if(isCurrentUAXIS() && (toBeHomed == 0))
				{
					currentUserPosition[X_AXIS] = moveBuffer.coords[U_AXIS];
				}
				if(!IsBitSet(toBeHomed, Z_AXIS) && isRaiseZHoming)
				{
					isRaiseZHoming = false;

					if(useMeshBeforeHomingZ != reprap.GetMove().IsUsingMesh())
					{
						if(useMeshBeforeHomingZ)
							LoadHeightMap(gb, reply);
						else
							ClearBedMapping();
					}
				}
	...
	case GCodeState::m109ToolChange0:	// Run tfree for the old tool (if any)
		...
		if ((gb.MachineState().toolChangeParam & TFreeBit) != 0)
		{
			...
			//Raise
			if (oldTool != nullptr && AllAxesAreHomed() && !gb.MachineState().ignore_FileMacro)
			{
				isRaiseToolFreeMove = true;
	...
	case GCodeState::m109ToolChange1:	// Release the old tool (if any), then run tpre for the new tool
		if (LockMovementAndWaitForStandstill(gb))		// wait for tfree.g to finish executing
		{
			...
			gb.AdvanceState();
			//Raise
			if (!gb.MachineState().ignore_FileMacro && reprap.GetTool(gb.MachineState().newToolNumber) != nullptr && AllAxesAreHomed() && (gb.MachineState().toolChangeParam & TPreBit) != 0)
			{
				String<ShortScratchStringLength> scratchString;
	...
	case GCodeState::m109ToolChange2:	// Select the new tool (even if it doesn't exist - that just deselects all tools) and run tpost
		if (LockMovementAndWaitForStandstill(gb))		// wait for tpre.g to finish executing
		{
			...
			{
				//Raise
				if (!gb.MachineState().ignore_FileMacro && reprap.GetTool(gb.MachineState().newToolNumber) != nullptr && (gb.MachineState().toolChangeParam & TPostBit) != 0)
				{
					String<ShortScratchStringLength> scratchString;
	...
	case GCodeState::m109ToolChangeComplete:
		if (LockMovementAndWaitForStandstill(gb))		// wait for tpost.g to finish executing
		{
			...
			if(isCurrentUAXIS())
			{
				//after change to RIGHT_X_CARRIAGE_TOOL need to refresh currentUserPosition
				currentUserPosition[X_AXIS] = moveBuffer.coords[U_AXIS];
			}
			gb.MachineState().ignore_FileMacro = false;
			isRaiseToolPosRestore = true;
#endif
			if (platform.Emulating() == Compatibility::raise)
			{
				platform.MessageF(UsbMessage,"echo:Active Extruder: %u\n", gb.MachineState().newToolNumber);
				platform.MessageF(UsbMessage, "ok:%" PRIu32 "\n", reprap.GetMove().GetScheduledMoves() - reprap.GetMove().GetCompletedMoves());
				updateFilamentStatus(gb.MachineState().newToolNumber);
			}
	...
	case GCodeState::m190WaitForBed:
		{
			float tolerance = TEMPERATURE_CLOSE_ENOUGH;
			if (cancelWait || simulationMode != 0 || reprap.GetHeat().HeaterAtSetTemperature(reprap.GetHeat().GetBedHeater(0), gb.MachineState().waitWhileCooling,tolerance))
			{
				cancelWait = isWaiting = false;
				gb.SetState(GCodeState::normal);
				if (platform.Emulating() == Compatibility::raise)
	...
	case GCodeState::gridProbing2b:		// ready to probe the current grid probe point
		if (LockMovementAndWaitForStandstill(gb))
		{
			...
			if((reprap.GetPlatform().Emulating() == Compatibility::raise))
			{
				platform.SetProbing(false);//down probe
			}
		...
		if (LockMovementAndWaitForStandstill(gb))
		{
			...
			else
			{
				...
				if((reprap.GetPlatform().Emulating() == Compatibility::raise))
				{
					...
					const float x = grid.GetXCoordinate(gridXindex);
					const float y = grid.GetYCoordinate(gridYindex);
					String<128> gridReply;
					gridReply.printf("gridXindex:%d gridYindex:%d HeightError:%.3f X:%.2f Y:%.2f\n", gridXindex, gridYindex, (double)g30zHeightError, (double)x, (double)y);
					platform.Message(UsbMessage, gridReply.c_str());
				}
	...
	case GCodeState::gridProbing7:
		...
		{
			float mean, deviation, flatness;
			const uint32_t numPointsProbed = reprap.GetMove().AccessHeightMap().GetStatistics(mean, deviation);
			flatness = reprap.GetMove().AccessHeightMap().GetFlatness();
			if((reprap.GetPlatform().Emulating() == Compatibility::raise)) //Rais3d Probe
			{
				platform.SetProbing(true);//up probe
			}
			if (numPointsProbed >= 4)
			{
				reply.printf("%" PRIu32 " points probed, mean error:%.3f, deviation:%.3f, flatness:%.3f\n", numPointsProbed, (double)mean, (double)deviation, (double)flatness);
	...
	case GCodeState::probingAtPoint2a:								// note we return to this state when doing the second and subsequent taps
		...
		{
			gb.AdvanceState();
			platform.SetProbing(false);  //new probe
			if (platform.GetZProbeType() == ZProbeType::blTouch)
			{
				DoFileMacro(gb, DEPLOYPROBE_G, false);				// bltouch needs to be redeployed prior to each probe point
			}
	...
	case GCodeState::probingAtPoint3:
		...
		if (millis() - lastProbedTime >= (uint32_t)(platform.GetCurrentZProbeParameters().recoveryTime * SecondsToMillis))
		{
			...
			else if (platform.GetZProbeResult() == EndStopHit::lowHit)		// check for probe already triggered at start
			{
				...
				platform.SetProbing(true);  //newprobe
				if (platform.GetZProbeType() != ZProbeType::none && !probeIsDeployed)
				{
					DoFileMacro(gb, RETRACTPROBE_G, false);
				}
			...
			else
			{
				...
				moveBuffer.endStopsToCheck = ZProbeActive;
#ifdef RAISE
				moveBuffer.coords[Z_AXIS] = -1.1 * platform.AxisTotalLength(Z_AXIS);
#else
	...
	case GCodeState::probingAtPoint5:
		...
		if (LockMovementAndWaitForStandstill(gb))
		{
			...
			gb.AdvanceState();
			platform.SetProbing(true); //newprobe Raise
			if (platform.GetZProbeType() != ZProbeType::none && !probeIsDeployed)
			{
				DoFileMacro(gb, RETRACTPROBE_G, false);
			}
...
void GCodes::StartNextGCode(GCodeBuffer& gb, const StringRef& reply)
{
			...
			)
	{
		...
		if(isRaiseUsbHeart)
		{
			if(serialInput->BytesCached() == 0 && 0 == MOVE_COUNTER)
			{
				if(now - raiseIdleTime > 200)
				{
					reprap.GetPlatform().MessageF(UsbMessage, "idle\n");
					raiseIdleTime = now;
				}
			}
			else
...
{
	if(Compatibility::raise == platform.Emulating())
	{
		// when detect move command report filament error, no need to report with endless cycle
		return;
	}
...
void GCodes::updateFilamentStatus(int toolNum)
{
	for(size_t i= 0; i < MaxExtruders; ++i)
	{
		frsTriggerCheck[i] = false;
	...
	if(2 == toolNum)
	{
		for(size_t i= 0; i < MaxExtruders; ++i)
		{
			if(lastFilamentStatus[i] != FilamentSensorStatus::ok)
			{
				lastFilamentError = lastFilamentStatus[i];
				lastFilamentErrorExtruder = toolNum;
				return;
			}
		}
	...
	{
		lastFilamentError = lastFilamentStatus[toolNum];
...
bool GCodes::isUAXISVisible()
{
	return numVisibleAxes > U_AXIS;
}

bool GCodes::isCurrentUAXIS()
{
	return isUAXISVisible() && (RIGHT_X_CARRIAGE_TOOL == reprap.GetCurrentToolNumber());
}
...
{
	if(frsTriggerCheck[extruder])
		frsCheckTimes[extruder]++;

	if(lastFilamentStatus[extruder] != fstat)
	{
		if(!frsTriggerCheck[extruder])
		{
			//start to check frs trigger
			frsCheckTimes[extruder] = 0;
			filterFilamentError[extruder] = 0;
			frsTriggerCheck[extruder] = true;
		}

		filterFilamentError[extruder]++;
	}
	else
	{
		if(!frsTriggerCheck[extruder])
	...
	if(frsTriggerCheck[extruder] && (frsCheckTimes[extruder] > 30000))// more than 5 seconds, stop check 5 seconds about 300000 times check
	{
		frsTriggerCheck[extruder] = false;
		int errorPercent = filterFilamentError[extruder] * 100 / frsCheckTimes[extruder];
		if(errorPercent < 90)// if percentage of frs error times less than 50, treat it as noise
			return;
	}
	else
		return;// less than 5 seconds continue caculate it

	lastFilamentStatus[extruder] = fstat;

	const Tool * const tool = reprap.GetCurrentTool();

	if(tool->Number() == (int)extruder)//only active extruder need to report filament run out
	{
		lastFilamentError = lastFilamentStatus[extruder];
		if(lastFilamentError != FilamentSensorStatus::ok)
		{
			lastFilamentErrorExtruder = extruder;
			String<100> filamentErrorString;
			filamentErrorString.printf("Custom: Filament Error T%u\nok:%u\n", lastFilamentErrorExtruder, (unsigned int)MOVE_COUNTER);
			platform.MessageF(UsbMessage, filamentErrorString.c_str());
		}
	}
	if(2 == tool->Number())
	{
		for(size_t i= 0; i < MaxExtruders; ++i)
		{
			...
			{
				lastFilamentError = lastFilamentStatus[i];
				lastFilamentErrorExtruder = tool->Number();
				String<100> filamentErrorString;
				filamentErrorString.printf("Custom: Filament Error T%u\nok:%u\n", lastFilamentErrorExtruder, (unsigned int)MOVE_COUNTER);
				platform.MessageF(UsbMessage, filamentErrorString.c_str());
				return;
...
#ifdef RAISE
	platform.Message(UsbMessage, stallErrorString.c_str());
#else
	platform.MessageF(UsbMessage, "Error: %s",stallErrorString.c_str());
#endif
...
//check whether X and U axis would crash
bool GCodes::IsXUAxisCrash(float x, float u)
{
	if(!isUAXISVisible())
		return false;
	if(!IsBitSet(axesHomed, X_AXIS) || !IsBitSet(axesHomed, U_AXIS))
		return false;

	if(u - x < (platform.AxisMinimum(U_AXIS) - platform.AxisMinimum(X_AXIS)))
		return true;

	return false;
}

//after switching tools, standby the x or u axis at specific point
bool GCodes::RaiseToolFree(int toolNum)
{
	if(!isUAXISVisible())
		return false;
	...
	if(0 == toolNum)
	{
		moveBuffer.coords[X_AXIS] = platform.AxisMinimum(X_AXIS) + 0.1;
	}
	else
	{
		moveBuffer.coords[U_AXIS] = platform.AxisMaximum(U_AXIS) - 0.1;
	}
	return true;
}
...
#ifdef RAISE
	if((fabs(currentUserPosition[Z_AXIS] - GetRaiseCurrentZPos()) > 1e-3) || (fabs(moveBuffer.coords[Z_AXIS] - GetRaiseCurrentZPos()) > 1e-3) || zMoveFlag)
	{
		moveBuffer.isZMoving = true;
	}
	else
		moveBuffer.isZMoving = false;
#endif
...
#ifdef RAISE
			if(isRaiseToolFreeMove)
			{
				RaiseToolFree(reprap.GetCurrentToolNumber());
			}
#endif
...
#ifdef RAISE
			if(LEFT_X_CARRIAGE_TOOL != reprap.GetCurrentToolNumber())
				ToolOffsetInverseTransform(moveBuffer.coords, currentUserPosition);//update user position for U_AXIS

			if(isUAXISVisible())
			{
				if((gb.Seen('X') || gb.Seen('U')) && IsXUAxisCrash(moveBuffer.coords[X_AXIS], moveBuffer.coords[U_AXIS]))
				{
					String<256> errstr;
					errstr.printf("Error: X and U axis will crash : %s xPos:%.2f uPos:%.2f\n", gb.Buffer(),(double)moveBuffer.coords[X_AXIS], (double)moveBuffer.coords[U_AXIS]);
					HandleReply(gb, GCodeResult::ok, errstr.c_str());
...
GCodeResult GCodes::DoHome(GCodeBuffer& gb, const StringRef& reply)
{
	...
	if(gb.Seen('P'))
	{
		// P1 home with probe ,P0 home with endstop,default is P1
		u_int32_t probeHoming = gb.GetUIValue();
		isRaiseHomeZProbe = probeHoming > 0? true : false;
	}
	else
		isRaiseHomeZProbe = true;
#endif

#ifdef RAISE_LARGE_MACHINE
	isRaiseHomeZProbe = false;
#endif
...
float GCodes::GetRaiseCurrentZPos()
{
	float liveCoordinates[MaxTotalDrivers];
	reprap.GetMove().LiveCoordinates(liveCoordinates, reprap.GetCurrentXAxes(), reprap.GetCurrentYAxes());
	const Tool * const currentTool = reprap.GetCurrentTool();
	...
	return liveCoordinates[Z_AXIS];
}

void GCodes::CurrentMoveCompleted(bool isZMoving)
{
	isZMovingCompleted = isZMoving;
}

void GCodes::GetRaiseCurrentCoordinates(const StringRef& s) const
{
	float liveCoordinates[MaxTotalDrivers];
	reprap.GetMove().LiveCoordinates(liveCoordinates, reprap.GetCurrentXAxes(), reprap.GetCurrentYAxes());
	const Tool * const currentTool = reprap.GetCurrentTool();
	...
	{
		// Don't put a space after the colon in the response, it confuses Pronterface
		//s.catf("%c:%.3f ", axisLetters[axis], HideNan(moveBuffer.coords[axis]));
		s.catf("%c:%.3f ", axisLetters[axis], HideNan(liveCoordinates[axis]));
	}

	s.catf("E:%.1f ", (double)virtualExtruderPosition); //Should be E0, but don't know whether is same as Marlin or not.
	...
	for (size_t i = 0; i < numVisibleAxes; ++i)
	{
		s.catf(" %c:%" PRIi32, axisLetters[i],reprap.GetMove().GetEndPoint(i));
	}
}
...
GCodeResult GCodes::ManageTool(GCodeBuffer& gb, const StringRef& reply)
{
	...
	if (seen)
	{
		...
		{
			Tool* tool1 = NULL;
			tool1 = Tool::Create(toolNumber, name.c_str(), drives, dCount, heaters, hCount, xMap, yMap, fanMap, reply);

			Tool* const tool=tool1;
			if (tool == nullptr)
			{
				return GCodeResult::error;
			}
...
void GCodes::HandleReply(GCodeBuffer& gb, GCodeResult rslt, const char* reply)
{
	...
	case Compatibility::reprapFirmware:
		...
		break;
	case Compatibility::raise:
		//Raise: Add support for M code
		//Don't add ok:scheduled_move for Raise 105,109,190,114,115,117,119,301

		if (0 == strncmp(reply, "Printing paused", strlen("Printing paused")))
		{
			platform.MessageF(type, "%s\n", reply);
		}
		else if (gb.GetCommandLetter() == 'M' && (!gb.HasCommandNumber()))
		{
			if (!reprap.GetMove().AllMovesAreFinished())
			{
				platform.MessageF(type, "ok:busy\n");
			}else
			{
				platform.MessageF(type, "ok:%u\n", (unsigned int)MOVE_COUNTER);
			}
		}
		else if (gb.GetCommandLetter() == 'G' && (gb.GetCommandNumber() == 28 || gb.GetCommandNumber() == 0 || gb.GetCommandNumber() == 1)
				&& lastFilamentError != FilamentSensorStatus::ok && reprap.GetFilamentSensorSwitch() && gb.Seen('E'))
		{
			String<100> filamentErrorString;
			filamentErrorString.printf("Custom: Filament Error T%u\nok:%u\n", lastFilamentErrorExtruder, (unsigned int)MOVE_COUNTER);
			platform.MessageF(type, filamentErrorString.c_str());
		}
		...
		else if (gb.GetCommandLetter() == 'M' && gb.GetCommandNumber() == 29)
		{
			platform.MessageF(type, "%s\n", reply);
			platform.MessageF(type, "ok:%u\n", (unsigned int)MOVE_COUNTER);
		}
		else if ((gb.GetCommandLetter() == 'M') && (gb.GetCommandNumber() == 112 || gb.GetCommandNumber() == 114 || gb.GetCommandNumber() == 119 || gb.GetCommandNumber() == 115 || gb.GetCommandNumber() == 301 || gb.GetCommandNumber() == 600))
		{
			platform.MessageF(type, "%s\nok:%u\n", reply,(unsigned int)MOVE_COUNTER);

		}
		else if (gb.GetCommandLetter() == 'M' && (gb.GetCommandNumber() == 105 || gb.GetCommandNumber() == 998 || gb.GetCommandNumber() == 109 || gb.GetCommandNumber() == 190  ))
		{
			platform.MessageF(type, "%s\n", reply);
		}
		...
		{
			platform.MessageF(type, "ok:%u\n", (unsigned int)MOVE_COUNTER);
#ifdef Raise_CMD_DEBUG
			platform.MessageF(type, "Cmd: %s\n", gb.Buffer());
#endif
		}
		break;
...
void GCodes::SetPidParameters(GCodeBuffer& gb, int heater, const StringRef& reply)
{
	...
	if (heater >= 0 && heater < (int)NumHeaters)
	{
		...
		if (seen)
		{
			...
			reply.printf("ok e:%u p:%.2f i:%.2f d:%.2f", heater, (double)pp.kP, (double)pp.kI, (double)pp.kD);
		}
				...
				const size_t inputAxis = (IsBitSet(explicitAxes, axis)) ? axis
										: (IsBitSet(xAxes, axis)) ? X_AXIS
											: (IsBitSet(yAxes, axis)) ? Y_AXIS
												: axis;
#ifdef RAISE
				//if move x and u axis without gcode command (like change currentUserPosition manually), dont need to convert x and u axis co
				if(IsBitSet(explicitAxes, inputAxis))
				{
					if(isUAXISVisible() && (axis == U_AXIS) && (DUPLICATE_MIRROR_TOOL == reprap.GetCurrentToolNumber()))
					{
						if((PrintMode::normal != raisePrintMode) && !IsBitSet(explicitAxes, U_AXIS) && IsBitSet(explicitAxes, X_AXIS))
						{
							if(PrintMode::duplicate == raisePrintMode)
							{
								coordsOut[axis] = ((coordsIn[X_AXIS] + raiseDuplicateOffset) * axisScaleFactors[axis]) + totalOffset;
							}
							else
							{
								//mirror mode
								coordsOut[axis] = ((platform.AxisMaximum(U_AXIS) - coordsIn[X_AXIS] + platform.AxisMinimum(X_AXIS)) * axisScaleFactors[axis]) + totalOffset;
							}
						}
						else
...
void GCodes::ToolOffsetInverseTransform(const float coordsIn[MaxAxes], float coordsOut[MaxAxes])
{
	...
	else
	{
		...
		for (size_t axis = 0; axis < numVisibleAxes; ++axis)
		{
			const float totalOffset = GetWorkplaceOffset(axis) - currentTool->GetOffset(axis);
#ifdef RAISE
			//map u currentUserPositon to x axis for right x-carriage
			if(isCurrentUAXIS() && (X_AXIS == axis))
				coordsOut[axis] = coordsIn[U_AXIS]/axisScaleFactors[axis] - totalOffset;
			else
				coordsOut[axis] = coordsIn[axis]/axisScaleFactors[axis] - totalOffset;
...
//Raise
// Store a standard-format temperature report in 'reply'. This doesn't put a newline character at the end.
void GCodes::GenerateRaiseTemperatureReport(const StringRef& reply) const
{
	Heat& heat = reprap.GetHeat();
	//reply.cat("ok ");
	//The following is believed to be compatible with Marlin and Octoprint, based on thread https://github.com/foosel/OctoPrint/issues/2590#issuecomment-385023980
	...
	{
		ReportToolTemperatures(reply, tool, true);
		reply.catf(" F%u:", tool->Number());
		reply.catf("%.1f",(double)(extrusionFactors[tool->Number()]*100.00));
	}
	...
	reply.catf(" S0:%.1f", (double)platform.GetFanValue(0)*255);//fan0
	reply.catf(" S1:%.1f", (double)platform.GetFanValue(1)*255);//fan1
	reply.catf(" P:%.1f", (double)(speedFactor));//Global Feedrate
}
...
void GCodes::CheckReportDue(GCodeBuffer& gb, const StringRef& reply) const
{
	...
	if (gb.timerRunning)
	{
		if (now - gb.whenTimerStarted >= 1000)
		{
			...
			if (platform.Emulating() == Compatibility::raise && (&gb == serialGCode || &gb == telnetGCode))
			{
				// In RaiseTouch emulation mode we should return a standard temperature report every second
				GenerateRaiseTemperatureReport(reply);
			    reply.cat('\n');
				platform.Message(UsbMessage, reply.c_str());
			}
