...
bool DDA::Init(GCodes::RawMove &nextMove, bool doMotorMapping)
{
	...
	if (doMotorMapping)
	{
		//Raise Convert Cartesian coordinates to motor steps
	...
	for (size_t drive = 0; drive < MaxTotalDrivers; drive++)
	{
		...
		if (drive < numTotalAxes)
		{
			...
			{
				endPoint[drive] = Move::MotorMovementToSteps(drive, nextMove.coords[drive]);// Raise Convert Cartesian coordinates to motor steps for moveType S2
			}
		...
		if (goingUp)
		{
			...
			if (laDDA->topSpeed >= laDDA->requestedSpeed)
			{
				...
				goingUp = false;
					 ...
					 && laDDA->prev->decelDistance > 0.0				// if the previous move has no deceleration phase then no point in adjusting it
					)
			{
...
//if (t3 < minCalcTime) minCalcTime = t3;
	...
	if (state == completed)
	{
		...
		if(isZMoving)
			reprap.GetGCodes().CurrentMoveCompleted(true);			// tell Gcode that the current move is complete, used to return z pos for power loss
