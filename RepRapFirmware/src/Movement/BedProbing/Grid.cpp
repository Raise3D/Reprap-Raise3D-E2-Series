...
//ax+by+cz=d
void HeightMap::EstimatePlane(float &a, float &b, float &c, float &d) const
{
	//calculating the bed plane by least squares fit
	//algorithm: http://www.ilikebigbits.com/blog/2015/3/2/plane-from-points
	float sumX = 0, sumY = 0, sumZ = 0;
	int n = 0;
	for (uint32_t iY = 0; iY < def.numY; iY++)
	...
	b *= normLenInv;
	c = normLenInv;
	d = centX*a + centY*b + centZ*c;
	d *= -1;
}

float HeightMap::GetFlatness() const
{
	float a = 0.0, b = 0.0, c = 0.0, d = 0.0;
	EstimatePlane(a, b, c, d);
	...
	for (uint32_t iY = 0; iY < def.numY; iY++)
	{
		for (uint32_t iX = 0; iX < def.numX; iX++)
		{
			//distance from point to fitting plane: dh = abs(ax+by+cz+d)/sqrt(a*a + b*b + c*c)
			const uint32_t index = GetMapIndex(iX, iY);
			if (IsHeightSet(index))
			{
				const float fX = (def.xSpacing * iX) + def.xMin;
				const float fY = (def.ySpacing * iY) + def.yMin;
				const float fZ = gridHeights[index];
				const double heightError = (a*fX + b*fY + c*fZ + d) / sqrt(a*a + b*b + c*c);
				if(maxZ < heightError)
				{
					maxZ = heightError;
				}
