...
#ifdef RAISE
	if(IsBitSet(toBeHomed, X_AXIS) && IsBitSet(toBeHomed, U_AXIS) && IsBitSet(toBeHomed, Y_AXIS))
	{
		filename.copy("homexuy.g");
		return 0;
	}
	if(IsBitSet(toBeHomed, X_AXIS) && IsBitSet(toBeHomed, U_AXIS))
	{
		filename.copy("homexu.g");
		return 0;
	}
	if(IsBitSet(toBeHomed, X_AXIS) && IsBitSet(toBeHomed, Y_AXIS))
	{
		filename.copy("homexy.g");
		return 0;
	}
#endif
