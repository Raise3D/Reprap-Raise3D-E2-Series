...
void PID::Spin()
{
	if (model.IsEnabled())
	{
		...
		else
		{
			...
			//platform.MessageF(GenericMessage,"heater %d: check\n",heater);



			//max temp check regardless of heater state
			if (temperature >= GetHighestTemperatureLimit())
			{
				lastPwm = 0.0;
				SetHeater(0.0);						// do this here just to be sure, in case the call to platform.Message causes a delay
				if (IsTuning())
				...
				mode = HeaterMode::fault;
				reprap.GetGCodes().HandleHeaterFault(heater);
				platform.MessageF(ErrorMessage, "MaxTempExceeded on heater %d\n", heater);
				reprap.FlagTemperatureFault(heater);
			}
...
void PID::StartAutoTune(float targetTemp, float maxPwm, const StringRef& reply)
{
	...
	if (!model.IsEnabled())
	{
		if(Compatibility::raise == reprap.GetPlatform().Emulating())
			reply.printf("PID Autotune failed! Heater %d cannot be auto tuned while it is disabled", heater);
		else
	...
	{
		if(Compatibility::raise == reprap.GetPlatform().Emulating())
			reply.printf("PID Autotune failed! Heater %d must be off and cold before auto tuning it", heater);
		else
	...
	else
	{
		...
		{
			if(Compatibility::raise == reprap.GetPlatform().Emulating())
				reply.printf("PID Autotune failed! Heater %d reported error '%s' at start of auto tuning", heater, TemperatureErrorString(err));
			else
		...
		else
		{
			...
			if(Compatibility::raise == reprap.GetPlatform().Emulating())
				reply.printf("PID Autotune start.Heater %d", heater);
			else
...
void PID::GetAutoTuneStatus(const StringRef& reply)	// Get the auto tune status or last result
{
	...
	{
		if(Compatibility::raise == reprap.GetPlatform().Emulating())
			reply.printf("PID Autotune failed!");
		else
...
void PID::DoTuningStep()
{
	...
	platform.MessageF(UsbMessage, "ok T:%.2f @:%.1f\n", (double)temperature, (double)tuningPwm);
...
void PID::CalculateModel()
{
	...
	if (tuned)
	{
		...
		if(Compatibility::raise == reprap.GetPlatform().Emulating())
		{
			...
			platform.MessageF(UsbMessage,"Kp: %.2f\nKi: %.2f\nKd: %.2f\nPID Autotune finished! Heater %d\n", (double)params.kP, (double)params.kI, (double)params.kD, heater);
		}
	}
	else
	{
