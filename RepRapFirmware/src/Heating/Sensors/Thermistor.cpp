...
#include "Gcodes/Gcodes.h" //Raise£ºto get emulating state, maybe affecting realtime performance, need to adjust.
...
//debugPrintf("vref=%" PRIi32 " vssa=%" PRIi32 " maxdrop=%" PRIi32 "\n", averagedVrefReading, averagedVssaReading, maxDrop);
	...
	if (tempFilter.IsValid())
	{
		...
		//debugPrintf("vref=%" PRIi32 " Tavg=%" PRIi32 "\n", averagedVrefReading,  averagedTempReading);
		//debugPrintf("denom=%.3f\n", (double)denom);
		if (isRaiseTC)
		{//Raise: use magic word 2004331123 to trigger Raise thermalcouple simulating. Otherwise thermistor will not be running correctly.
			t = 330.0 * (float)averagedTempReading /16323;//16523//17423;//15153.0;
			//debugPrintf("vref=%" PRIi32 " Tavg=%" PRIi32 "\n", averagedVrefReading,  averagedTempReading);
			//debugPrintf("t=%.3f\n", (double)t);
